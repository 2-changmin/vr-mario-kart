...
2. **main에 직접 push 금지** — 브랜치 → PR → (가능하면 리뷰) → 머지 (PR 필수는 GitHub에서 강제됨, 승인 자체는 필수 아님 — 2026-09-17 변경)
...
- ⚠️ 상대방 승인은 **더 이상 GitHub이 강제하지 않음** (2026-09-17부터) — 그래도 가능하면 상대방 리뷰를 받고 머지할 것

### 머지 방식

- **Squash and merge만 허용** → main 히스토리가 PR 단위로 깔끔하게 남음
- 머지 커밋 제목은 PR 제목 그대로 사용
- 머지는 **PR 작성자**가 가능하면 리뷰를 받은 뒤 직접 수행 (급하면 승인 없이도 머지 가능)
...
- [ ] 가능하면 상대방 리뷰 받고 Squash Merge, 이슈 자동 종료 확인
